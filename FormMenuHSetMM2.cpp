...
void FormMenuHSetMM2::init(FormA *f, E_MotorMode mm)
{
    form = f;
    mptrw = new MPTRWidget(this, &form->mac, MMP.Motor2, form->mpri);
    mptrw->start_later = true;

    edits[0] = ui->val_1; mps[0] = MP.motor2_sp;
    edits[1] = ui->val_2; mps[1] = MP.motor2_ta;
    edits[2] = ui->val_3; mps[2] = MP.motor2_tm;
    edits[3] = ui->val_4; mps[3] = MP.motor2_tb;
    edits[4] = ui->val_5; mps[4] = MP.motor2_ts;
...
{
    if (areas.isCon(MMP.Motor2))
        QTimer::singleShot(0, this, SLOT(refreshUI()));
}

void FormMenuHSetMM2::onModeClicked(int i)
{
    motormm = i;
    motormi = 0;
...
void FormMenuHSetMM2::onEditFinished()
{
    if (!isCorrectMode())
        return;
...
void FormMenuHSetMM2::on_btn_add_1_clicked()
{
    if (!isCorrectMode())
        return;
...
void FormMenuHSetMM2::on_btn_add_2_clicked()
{
    if (!isCorrectMode())
        return;
...
void FormMenuHSetMM2::on_btn_del_clicked()
{
    if (!isCorrectMode())
        return;
...
void FormMenuHSetMM2::on_val_curi_editingFinished()
{
...
void FormMenuHSetMM2::on_gm_clicked(int i)
{
    QVector<int> v = getMMData();
...
void FormMenuHSetMM2::on_btn_copy_clicked()
{
...
void FormMenuHSetMM2::on_btn_test_clicked()
{
    int id = btg->checkedId();
    if (id < 0)
        return;
...
QVector<int> FormMenuHSetMM2::getMMData()
{
    QVector<int> v;
    for (int k = 0; k < 10; k++) {
        int d[m_datan];
...
void FormMenuHSetMM2::setMMData(const QVector<int> &v)
{
...
void FormMenuHSetMM2::adjustMMData(int mi, int *d, int size)
{
    Q_UNUSED(mi); Q_UNUSED(d); Q_UNUSED(size);
}
